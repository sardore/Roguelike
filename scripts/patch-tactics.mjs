...
if (!source.includes("status.id === 'charging'")) {
  replaceRegex(
    /function executeMonsterAbility\(state: GameState, monster: MonsterEntity, rng: DeterministicRng\): boolean \{[\s\S]*?\n\}\nfunction monsterTurn/,
...
if (!source.includes("status.id === 'winding'")) {
  replaceRegex(
...
if (!source.includes('strikes from reach')) {
  replaceRegex(
    /    const monster = monsterAt\(state, target\.x, target\.y\);[\s\S]*?    accepted = true;\n  \} else if \(action\.type === 'wait'\) accepted = true;/,
