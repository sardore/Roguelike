...
export interface RogueStatus{
  nutrition:number;maxNutrition:number;foodState:'SATIATED'|'OK'|'HUNGRY'|'WEAK'|'STARVING';gold:number;piety:number;
  intrinsics:Partial<Record<IntrinsicKind,number>>;identified:ItemKind[];
}
...
interface Meta{
  ...
  intrinsics:Partial<Record<IntrinsicKind,number>>;identified:Set<ItemKind>;
  features:DungeonFeature[];corpses:Corpse[];
  lastFloor:number;lastTurn:number;lastEnemies:Unit[];lastEnemyPos:Map<Unit,string>;lastX:number;lastY:number;lastBag:ItemKind[];
  warnedFood:string;
}

const META=new WeakMap<TowerState,Meta>();
const D4=[[1,0],[-1,0],[0,1],[0,-1]] as const;
const ITEM_APPEARANCE:Record<ItemKind,string>={potion:'crimson glass vial',bomb:'wax-sealed clay sphere',ration:'wrapped travel ration',ward:'etched grey stone',smoke:'clouded glass ampoule',knife:'balanced throwing knife'};
...
function makeEnemyPos(enemies:Unit[]){const m=new Map<Unit,string>();for(const e of enemies)m.set(e,`${e.x},${e.y}`);return m}
function makeMeta(s:TowerState):Meta{return{nutrition:820,maxNutrition:1000,gold:18,piety:0,prayerCooldown:0,intrinsics:{},identified:new Set<ItemKind>(['ration','knife']),features:[],corpses:[],lastFloor:s.floor,lastTurn:s.turn,lastEnemies:[...s.enemies],lastEnemyPos:makeEnemyPos(s.enemies),lastX:s.hero.x,lastY:s.hero.y,lastBag:[...s.hero.bag],warnedFood:'OK'}}
...
export function populateNethackFloor(s:TowerState){const m=meta(s);m.features=[];m.corpses=[];placeClosedDoors(s);placeSecretDoors(s);placeGeneralFeatures(s);m.lastFloor=s.floor;m.lastX=s.hero.x;m.lastY=s.hero.y;m.lastEnemies=[...s.enemies];m.lastEnemyPos=makeEnemyPos(s.enemies)}
...
export function getRogueStatus(s:TowerState):RogueStatus{const m=meta(s);return{nutrition:m.nutrition,maxNutrition:m.maxNutrition,foodState:foodState(m),gold:m.gold,piety:m.piety,intrinsics:{...m.intrinsics},identified:[...m.identified]}}
export function getItemDisplayName(s:TowerState,k:ItemKind){return meta(s).identified.has(k)?itemKnownName(k):ITEM_APPEARANCE[k]}
export function isItemIdentified(s:TowerState,k:ItemKind){return meta(s).identified.has(k)}
export function hungerAttackPenalty(s:TowerState){const st=foodState(meta(s));return st==='STARVING'?2:st==='WEAK'?1:0}
...
export function systemOpaque(s:TowerState,x:number,y:number){const f=featureAt(s,x,y);return !!f&&f.kind==='door'&&!f.open}
...
export function preMoveSystem(s:TowerState,x:number,y:number,dx:number,dy:number):SystemMoveResult{
  ...
  if(f.kind==='door'&&!f.open){if(f.locked)return{flow:'spent',message:'The door is locked. ACT can force or work the lock.'};f.open=true;return{flow:'spent',message:'You open the door.'}}
  if(f.kind==='boulder'){
...
export function searchSystem(s:TowerState){const m=meta(s);let found=0;for(const f of m.features){if(f.revealed)continue;const d=Math.abs(f.x-s.hero.x)+Math.abs(f.y-s.hero.y);if(d>2)continue;const bonus=(s.hero.cls==='ranger'?.28:.08)+((m.intrinsics['keen-sense']??0)>0?.24:0);if(roll(s,4100+f.x*17+f.y*29)<.46+bonus){f.revealed=true;found++;if(f.kind==='secret-door'){const t=at(s,f.x,f.y);if(t){t.kind='door';t.seen=true}f.kind='door'}}}
...
function chestAction(s:TowerState,f:DungeonFeature){if(f.spent)return'The chest is empty.';f.spent=true;const m=meta(s),r=roll(s,4500+f.x*7+f.y*13);if(r<.16){s.hero.hp=Math.max(1,s.hero.hp-4);m.gold+=8;return'A needle trap snaps from the chest. Beneath it: 8 gold.'}const pool:ItemKind[]=['potion','ration','ward','smoke','knife','bomb'];const item=pool[Math.min(pool.length-1,Math.floor(roll(s,4501+f.x)*pool.length))]??'ration';s.hero.bag.push(item);m.gold+=4+Math.floor(roll(s,4502+f.y)*8);return`The chest yields ${getItemDisplayName(s,item)} and a handful of old coin.`}
function merchantAction(s:TowerState,f:DungeonFeature){const m=meta(s),stock=f.stock??[];if(!stock.length)return'The merchant has packed away the last useful thing.';const item=stock[0]!;const price=(f.price??16)+(item==='bomb'||item==='ward'?4:0);if(m.gold<price)return`The merchant taps ${price}g into the dust. You only have ${m.gold}g.`;m.gold-=price;s.hero.bag.push(item);stock.shift();f.stock=stock;return`You buy ${getItemDisplayName(s,item)} for ${price}g.`}
function itemKnownName(k:ItemKind){return k==='potion'?'Red Draught':k==='bomb'?'Cinder Bomb':k==='ration'?'Field Ration':k==='ward'?'Stone Ward':k==='smoke'?'Smoke Phial':'Throwing Knife'}
...
function snapshot(s:TowerState){const m=meta(s);m.lastFloor=s.floor;m.lastTurn=s.turn;m.lastEnemies=[...s.enemies];m.lastEnemyPos=makeEnemyPos(s.enemies);m.lastX=s.hero.x;m.lastY=s.hero.y;m.lastBag=[...s.hero.bag]}
...
function enteredHeroFeature(s:TowerState){const f=featureAt(s,s.hero.x,s.hero.y);if(!f)return;if(f.kind==='trap'){f.revealed=true;const d=3+Math.floor(roll(s,5200+f.x*3+f.y)*4);s.hero.guard=0;s.hero.hp-=d;log(s,`A hidden floor trap snaps shut for ${d}. SEARCH can reveal these before you step on them.`);if(s.hero.hp<=0){s.hero.hp=0;s.dead=true}}else if(f.kind==='pit'){s.hero.hp=Math.max(1,s.hero.hp-3);log(s,'You stumble into a shallow pit. A boulder could plug it.')}else if(f.kind==='fountain')log(s,'Water murmurs here. ACT to drink.');else if(f.kind==='altar')log(s,'An old altar waits in the dust. ACT to pray.');else if(f.kind==='chest'&&!f.spent)log(s,'A travel chest sits here. ACT to open it.');else if(f.kind==='merchant')log(s,'A silent merchant watches you. ACT to trade.');else if(f.kind==='engraving'&&!f.spent)log(s,'Something is engraved underfoot. ACT to read it.');else if(f.kind==='gold'&&!f.spent)log(s,'Old coin glints under the dust. ACT to gather it.')}
function triggerMonsterTraps(s:TowerState){const m=meta(s);for(const e of [...s.enemies]){const old=m.lastEnemyPos.get(e);if(old===`${e.x},${e.y}`)continue;const f=featureAt(s,e.x,e.y,'trap');if(!f)continue;f.revealed=true;const dmg=3+Math.floor(roll(s,5400+e.x*23+e.y*31)*5);e.hp-=dmg;if(at(s,e.x,e.y)?.visible)log(s,`${itemKnownEnemy(e.kind)} triggers a floor trap for ${dmg}.`);if(e.hp<=0){s.enemies=s.enemies.filter(q=>q!==e);awardKill(s,e);if(at(s,e.x,e.y)?.visible)log(s,`${itemKnownEnemy(e.kind)} dies in the trap.`)}}}
function itemKnownEnemy(k:EnemyKind){return k==='rat'?'The rat':k==='hound'?'The hound':k==='slime'?'The slime':k==='archer'?'The archer':k==='guard'?'The guard':k==='knight'?'The knight':k==='golem'?'The golem':k==='wisp'?'The wisp':k==='seer'?'The seer':k==='cultist'?'The curator':'The guardian'}
...
export function syncNethack(s:TowerState){const m=meta(s);if(s.floor!==m.lastFloor){populateNethackFloor(s);m.lastTurn=s.turn;m.lastBag=[...s.hero.bag];snapshot(s);return}
  ...
  const used=removedItem(m.lastBag,s.hero.bag);if(used){m.identified.add(used);if(used==='ration')m.nutrition=Math.min(m.maxNutrition,m.nutrition+260)}
  const delta=Math.max(0,s.turn-m.lastTurn);tickFood(s,delta);
  if(s.hero.x!==m.lastX||s.hero.y!==m.lastY)enteredHeroFeature(s);
  triggerMonsterTraps(s);
  snapshot(s)
}
