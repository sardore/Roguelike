...
function passable(s:GameState,x:number,y:number){const t=at(s,{x,y}); return !!t&&t.kind!=='wall';}
...
function enemyTurn(s:GameState,e:Enemy,rng:Rng){if(!s.enemies.includes(e)||s.over)return;const d=dist(e,s.player);const def=ENEMIES[e.kind];
  if(e.kind==='vapor-hound'){
    if(e.telegraph){const p=e.telegraph;e.telegraph=undefined;if(p.x===s.player.x&&p.y===s.player.y)hurtPlayer(s,def.damage,'Corrosive vapor burns you');const t=at(s,p);if(t&&t.kind==='floor')t.kind='acid';e.cooldown=3;return;}
  ...
  if(e.kind==='distiller-rat'){
    const onFire=dirs.some(([dx,dy])=>at(s,{x:e.x+dx,y:e.y+dy})?.kind==='fire'); if(onFire){stepToward(s,e,{x:e.x+(e.x-s.player.x),y:e.y+(e.y-s.player.y)},rng);return;}
  }
  if(d<=9)stepToward(s,e,s.player,rng);
}
...
export function move(s:GameState,dx:number,dy:number){if(s.over)return;const rng=new Rng((s.seed+s.turn*2654435761)>>>0);const x=s.player.x+dx,y=s.player.y+dy;if(!passable(s,x,y))return;const enemy=enemyAt(s,x,y);if(enemy)hurtEnemy(s,enemy,4);else{s.player.x=x;s.player.y=y;const item=s.items.find(i=>i.x===x&&i.y===y);if(item){s.player.inventory.push(item.kind);s.items=s.items.filter(i=>i!==item);push(s,`Picked up ${item.kind.replaceAll('-',' ')}.`,'good');}const t=at(s,s.player);if(t?.kind==='stairs'&&s.enemies.length===0){s.won=true;s.over=true;push(s,'The brass stair turns beneath your weight. The lower market is open.','odd');}}
  advance(s,rng);
}
function rayTarget(s:GameState,dx:number,dy:number,range=5):Point{let p={x:s.player.x,y:s.player.y};for(let i=0;i<range;i++){const n={x:p.x+dx,y:p.y+dy};const t=at(s,n);if(!t||t.kind==='wall')break;p=n;if(enemyAt(s,p.x,p.y))break;}return p;}
export function useItem(s:GameState,index:number,dir:Point={x:0,y:-1}){if(s.over)return;const kind=s.player.inventory[index];if(!kind)return;const rng=new Rng((s.seed^s.turn^index)>>>0);if(kind==='blue-tonic'){s.player.hp=Math.min(s.player.maxHp,s.player.hp+6);push(s,'The tonic closes the worst cuts. Something medicinal follows you.','odd');s.player.statuses.push({id:'marked',turns:8});}
  else if(kind==='red-phial'){const p=rayTarget(s,dir.x,dir.y);const t=at(s,p);if(t){t.kind='fire';push(s,'The red phial bursts into a low, hungry flame.','odd');for(const [dx,dy] of [[0,0],...dirs] as const){const q={x:p.x+dx,y:p.y+dy};const qt=at(s,q);if(qt?.kind==='acid')qt.kind='fire';const e=enemyAt(s,q.x,q.y);if(e)hurtEnemy(s,e,3);}}}
  else if(kind==='salt-bomb'){const p=rayTarget(s,dir.x,dir.y);push(s,'Salt cracks outward in a white ring.','good');for(const [dx,dy] of [[0,0],...dirs] as const){const q={x:p.x+dx,y:p.y+dy};const t=at(s,q);if(t&&(t.kind==='acid'||t.kind==='fire'))t.kind='floor';const e=enemyAt(s,q.x,q.y);if(e)hurtEnemy(s,e,4);}}
  else if(kind==='chalk'){const t=at(s,s.player);if(t){t.variant=15;push(s,'You draw a clean white line across the grime.','odd');}}
...
export function inspectAt(s:GameState,p:Point):string{const e=enemyAt(s,p.x,p.y);if(e)return `${ENEMIES[e.kind].name} — ${ENEMIES[e.kind].note}`;const t=at(s,p);if(!t)return'';return `${t.room??'stone'} / ${t.kind}`;}
