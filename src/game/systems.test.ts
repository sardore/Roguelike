...
describe('alchemical interactions',()=>{
  it('throws a red phial onto the tapped tile instead of maximum range',()=>{
    const s=createWorld(hash('exact-throw'));const target={x:10,y:12};useItem(s,0,target);expect(s.tiles[target.y*s.width+target.x]?.kind).toBe('fire');
  });
  it('burns a flammable fixture when hit',()=>{
    const s=createWorld(hash('burn-fixture'));const t=s.tiles[12*s.width+10]!;t.fixture='crate';t.blocks=true;useItem(s,0,{x:10,y:12});expect(t.kind).toBe('fire');expect(t.fixture).toBeUndefined();expect(t.blocks).toBe(false);
  });
