...
function stage1(s:GameState,c:Point,rng:Rng){fixture(s,c,'pressure-console',true,0);line(s,c,'brine',rng.chance(.5));corners(s,c,'glass');fixture(s,{x:c.x-2,y:c.y},'barrel',true,1);fixture(s,{x:c.x+2,y:c.y},'sealed-cache',true,0);spawn(s,c,'brine-warden','setpiece-warden','salt-abbot',7);spawn(s,c,'distiller-rat','setpiece-rat');drop(s,c,'frost-salts','setpiece-frost')}
function stage2(s:GameState,c:Point,rng:Rng){fixture(s,c,'scent-burner',true,0);cross(s,c,'miasma',3);corners(s,c,'sludge');fixture(s,{x:c.x-2,y:c.y},'sealed-urn',true,0);fixture(s,{x:c.x+2,y:c.y},'field-kit',true,0);spawn(s,c,'miasma-moth','setpiece-moth','embalmer',5);spawn(s,c,'homunculus','setpiece-hom');drop(s,c,rng.chance(.5)?'neutralizer':'smoke-ampoule','setpiece-cleaner')}
function stage3(s:GameState,c:Point,rng:Rng){fixture(s,c,'glass-organ',true,0);line(s,c,'oil',rng.chance(.5));corners(s,c,'embers',4);fixture(s,{x:c.x-2,y:c.y},'furnace',true,0);fixture(s,{x:c.x+2,y:c.y},'crucible',true,0);spawn(s,c,'soot-sprite','setpiece-heart','furnace-heart',8);spawn(s,c,'gutter-alchemist','setpiece-alch');drop(s,c,'amber-elixir','setpiece-amber')}
function stage4(s:GameState,c:Point,rng:Rng){fixture(s,c,'observation-desk',true,0);cross(s,c,'crystal');corners(s,c,'glass');fixture(s,{x:c.x-2,y:c.y},'silver-mirror',true,0);fixture(s,{x:c.x+2,y:c.y},'resonator',true,0);spawn(s,c,'glass-sentinel','setpiece-mirror','mirror-hunter',10);spawn(s,c,'miasma-moth','setpiece-moth4');drop(s,c,rng.chance(.5)?'solvent':'black-catalyst','setpiece-glass-tool')}
function stage5(s:GameState,c:Point,rng:Rng){fixture(s,c,'resonator',true,0);cross(s,c,'rune');line(s,c,rng.chance(.5)?'embers':'acid',false);fixture(s,{x:c.x-2,y:c.y},'relic-pedestal',true,0);fixture(s,{x:c.x+2,y:c.y},'sealed-urn',true,0);spawn(s,c,'crucible-knight','setpiece-executor','brass-executor',16);spawn(s,c,'vapor-hound','setpiece-hound',undefined,4);drop(s,c,RARE[rng.int(0,RARE.length-1)]!,'setpiece-final-reward')}

function districtMaster(s:GameState,c:Point){
  if((s.floorInDistrict??1)!==2)return;
  const id=`district-boss-${s.districtStage}`;
  if(s.districtStage===1)spawn(s,c,'brine-warden',id,'salt-abbot',16);
  else if(s.districtStage===2)spawn(s,c,'gutter-alchemist',id,'acid-seer',14);
  else if(s.districtStage===3)spawn(s,c,'crucible-knight',id,'furnace-heart',20);
  else if(s.districtStage===4)spawn(s,c,'glass-sentinel',id,'mirror-hunter',20);
  else spawn(s,c,'crucible-knight',id,'brass-executor',30);
}

export function applyDistrictSetpiece(s:GameState){
  const floor=s.floorInDistrict??1,key=`setpiece-${s.districtStage}-${floor}`;s.expansionFlags??=[];if(s.expansionFlags.includes(key))return;
  const rng=new Rng((s.seed^Math.imul(s.districtStage,0x27d4eb2d)^Math.imul(floor,0x165667b1)^0x9e3779b9)>>>0),c=choose(s,rng);if(!c)return;
