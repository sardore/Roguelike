...
describe('multi-district run volume',()=>{
  ...
  it('uses two floors per district and preserves carried loot',()=>{const g=new Game('ten-depths');g.state.player.inventory.push('amber-elixir');for(let i=0;i<9;i++){exitStep(g);expect(g.state.player.inventory).toContain('amber-elixir');expect(g.state.over).toBe(false)}expect(g.state.districtStage).toBe(5);expect(g.state.floorInDistrict).toBe(2);exitStep(g);expect(g.state.over).toBe(true);expect(g.state.won).toBe(true)});
  it('places a district master on every second floor',()=>{const g=new Game('boss-floor');exitStep(g);expect(g.state.floorInDistrict).toBe(2);expect(g.state.enemies.some(e=>e.id==='district-boss-1')).toBe(true)});
});
