...
export type FixtureKind = 'shelf' | 'still' | 'crate' | 'lamp' | 'planter' | 'boards' | 'herbs' | 'grate';
export type Tile = { kind: TileKind; room?: string; variant: number; discovered: boolean; visible: boolean; fixture?: FixtureKind; blocks?: boolean };
...
export type GameState = { width: number; height: number; tiles: Tile[]; player: Player; enemies: Enemy[]; items: GroundItem[]; turn: number; messages: Message[]; seed: number; over: boolean; won: boolean; noise: Point[] };
