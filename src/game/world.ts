...
export function createWorld(seed:number):GameState {
  ...
  // A legible six-tile-wide street. Buildings sit beside it instead of overwriting it.
  fillRect(tiles,1,10,35,6,'floor','apothecaries-row',rng);
  // Small recesses break the ruler-straight silhouette without cutting into buildings.
  fillRect(tiles,12,8,4,2,'floor','north-alley',rng);
  fillRect(tiles,1,9,4,1,'floor','apothecaries-row',rng);
  fillRect(tiles,32,9,4,1,'floor','apothecaries-row',rng);

  // Herbalist — front shop and rear preparation room.
  ...
  // Narrow service alley between the herbalist and distillery.
  fillRect(tiles,12,2,4,8,'floor','north-alley',rng);
  fixture(tiles,13,4,'barrel');
  fixture(tiles,14,7,'pipe',false);

  // Distillery — broad industrial room with a continuous facade.
  ...
  // Physic courtyard south of the street.
  fillRect(tiles,3,17,12,6,'floor','courtyard',rng);
  wallRect(tiles,2,16,14,8,'courtyard-wall',rng);
  set(tiles,8,16,'door','courtyard-door',2);
  set(tiles,15,19,'door','courtyard-door',1);
  fillRect(tiles,16,18,3,4,'floor','service-passage',rng);

  // Boarded apothecary on the south-east edge.
  fillRect(tiles,28,17,7,7,'floor','sealed-shop',rng);
  wallRect(tiles,27,16,9,9,'sealed-shop-wall',rng);
  set(tiles,31,16,'door','sealed-door',5);
  set(tiles,27,20,'door','sealed-door',5);
  fillRect(tiles,23,15,4,6,'floor','service-passage',rng);
  set(tiles,33,22,'stairs','sealed-shop',0);

  // Gutters and spills communicate use and neglect.
  for(const [x,y] of [[4,14],[10,15],[17,14],[24,15],[33,14]] as const)
    set(tiles,x,y,'water','street-drain',rng.int(0,4));
  for(const [x,y] of [[19,4],[20,4],[21,4],[21,5],[31,19],[32,19],[31,20]] as const)
    set(tiles,x,y,'acid','spill',rng.int(0,3));

  // Herbalist vocabulary.
  ...
  fixture(tiles,21,7,'table');

  // Courtyard vocabulary.
  fixture(tiles,5,18,'planter'); fixture(tiles,9,18,'planter');
  fixture(tiles,5,21,'planter'); fixture(tiles,12,21,'crate');
  fixture(tiles,11,18,'fountain'); fixture(tiles,4,21,'barrel');

  // Sealed shop vocabulary.
  fixture(tiles,29,18,'crate'); fixture(tiles,33,18,'shelf');
  fixture(tiles,28,20,'boards'); fixture(tiles,33,20,'cabinet');
  fixture(tiles,34,21,'crate'); fixture(tiles,29,22,'rubble',false);

  // Street furniture makes the facade readable at the opening camera position.
  fixture(tiles,2,13,'cart');
  fixture(tiles,6,10,'awning',false); fixture(tiles,9,10,'sign',false);
  fixture(tiles,20,10,'awning',false); fixture(tiles,24,10,'sign',false);
  fixture(tiles,4,11,'lamp',false); fixture(tiles,15,11,'lamp',false); fixture(tiles,28,12,'lamp',false);
  fixture(tiles,14,13,'crate'); fixture(tiles,34,13,'barrel');
  fixture(tiles,9,14,'grate',false); fixture(tiles,18,14,'grate',false); fixture(tiles,26,13,'grate',false);
  fixture(tiles,24,17,'pipe',false); fixture(tiles,26,18,'pipe',false);

  const state:GameState = {
    width:W, height:H, tiles,
    player:{x:8,y:12,hp:20,maxHp:20,guard:0,inventory:['red-phial','salt-bomb'],statuses:[]},
    enemies:[
      {id:'mite-a',kind:'glass-mite',x:9,y:4,hp:5,cooldown:0},
      {id:'rat-a',kind:'distiller-rat',x:24,y:4,hp:9,cooldown:0},
      {id:'hound-a',kind:'vapor-hound',x:31,y:21,hp:13,cooldown:1}
    ],
    items:[
      {id:'tonic-a',kind:'blue-tonic',x:4,y:4},
      {id:'chalk-a',kind:'chalk',x:18,y:6},
      {id:'salt-b',kind:'salt-bomb',x:31,y:18}
    ],
    turn:0,
    messages:[{text:'Apothecaries’ Row is quiet. Copper pipes click somewhere behind the shutters.',tone:'odd'}],
...
function isSurfaceWall(state:GameState,x:number,y:number,t:Tile):boolean {
  if(t.kind!=='wall'||t.room!=='old-city') return true;
  for(const [dx,dy] of [[1,0],[-1,0],[0,1],[0,-1]] as const){
    const n=at(state,{x:x+dx,y:y+dy});
    if(n&&n.kind!=='wall') return true;
  }
  return false;
}
...
export function updateVisibility(state:GameState){
  ...
  for(let y=Math.max(0,state.player.y-r);y<=Math.min(state.height-1,state.player.y+r);y++){
    for(let x=Math.max(0,state.player.x-r);x<=Math.min(state.width-1,state.player.x+r);x++){
      ...
      if(!t||!isSurfaceWall(state,x,y,t)) continue;
      if(!canSee(state,state.player.x,state.player.y,x,y)) continue;
      t.visible=true;t.discovered=true;
    }
  }
}
