...
function backgroundCeiling(c:CanvasRenderingContext2D,s:GameState){
  // Preserve all brick, pipe and material detail from the rich renderer. Only cap the peak contrast on
  // ordinary unoccupied architecture so hazards, fixtures and actors remain one visual step forward.
  for(let y=0;y<s.height;y++)for(let x=0;x<s.width;x++){
    const t=s.tiles[y*s.width+x];if(!t?.visible||t.fixture||occupied(s,x,y))continue;
    const X=x*TS,Y=y*TS;
    if(t.kind==='wall')px(c,X,Y,TS,TS,'#141712',.055);
    else if(t.kind==='floor')px(c,X,Y,TS,TS,'#151813',.035);
  }
}
...
function hazardContours(c:CanvasRenderingContext2D,s:GameState){
  for(let y=0;y<s.height;y++)for(let x=0;x<s.width;x++){
    ...
    if(!n?.visible||n.kind!==t.kind)line(c,X+3,Y+3,X+29,Y+3,col,2,.38);
    if(!so?.visible||so.kind!==t.kind)line(c,X+3,Y+29,X+29,Y+29,col,2,.38);
    if(!w?.visible||w.kind!==t.kind)line(c,X+3,Y+3,X+3,Y+29,col,2,.31);
    if(!e?.visible||e.kind!==t.kind)line(c,X+29,Y+3,X+29,Y+29,col,2,.31);
  }
}
...
function actorReadability(c:CanvasRenderingContext2D,s:GameState){
  for(const e of s.enemies){
    ...
    // A dark foot separation reads as part of the sprite rather than a UI box.
    px(c,X+6,Y+29,20,2,'#050705',.82);
  ...
  const X=s.player.x*TS,Y=s.player.y*TS,col='#f0e5c2';
  line(c,X+5,Y+5,X+11,Y+5,col,2,.56);line(c,X+5,Y+5,X+5,Y+11,col,2,.56);
  line(c,X+27,Y+5,X+21,Y+5,col,2,.56);line(c,X+27,Y+5,X+27,Y+11,col,2,.56);
  line(c,X+5,Y+27,X+11,Y+27,col,2,.46);line(c,X+5,Y+27,X+5,Y+21,col,2,.46);
  line(c,X+27,Y+27,X+21,Y+27,col,2,.46);line(c,X+27,Y+27,X+27,Y+21,col,2,.46);
}

function itemGleam(c:CanvasRenderingContext2D,s:GameState){
  for(const i of s.items){const t=s.tiles[i.y*s.width+i.x];if(!t?.visible)continue;const X=i.x*TS,Y=i.y*TS;px(c,X+25,Y+6,2,2,'#f1e7c4',.76);px(c,X+26,Y+4,1,6,'#f1e7c4',.38);px(c,X+24,Y+7,6,1,'#f1e7c4',.36)}
}

export function drawMap(canvas:HTMLCanvasElement,s:GameState){
  baseDrawMap(canvas,s);const c=canvas.getContext('2d')!;
  backgroundCeiling(c,s);hazardContours(c,s);itemGleam(c,s);actorReadability(c,s);
}
export { screenToTile };
