...
function hash(x:number,y:number,stage:number){let n=Math.imul(x*37+y*61+stage*97,0x45d9f3b);n=(n^(n>>>16))>>>0;return n}
...
function trimUnexploredSpill(c:CanvasRenderingContext2D,s:GameState){
  for(let y=0;y<s.height;y++)for(let x=0;x<s.width;x++){
    ...
    // Anything outside current sight should stay visually quiet. This only erases old decorative overdraw.
    if(w&&!w.visible)px(c,X-7,Y,7,TS,'#050705',1);
    if(e&&!e.visible)px(c,X+TS,Y,7,TS,'#050705',1);
  }
}
...
function finalBackgroundPass(c:CanvasRenderingContext2D,s:GameState){
  ...
  for(let y=0;y<s.height;y++)for(let x=0;x<s.width;x++){
    ...
    if(t.kind==='floor'&&!occupied(s,x,y)){
      ...
      // Sparse macro wear: one broad patch instead of many tiny cracks.
      if((hash(Math.floor(x/2),Math.floor(y/2),s.districtStage)%19)===0){px(c,X+6,Y+20,18,5,'#151812',.26);px(c,X+9,Y+20,10,1,p.edge,.10)}
      if(t.fixture&&!ACTIONABLE.has(t.fixture))quietFixture(c,s,x,y,t);
    }
  }
}

function wallLandmarks(c:CanvasRenderingContext2D,s:GameState){
  const p=palette(s);
  for(let y=0;y<s.height-1;y++)for(let x=0;x<s.width;x++){
    ...
    if(hash(x,y,s.districtStage)%17!==0)continue;
    const X=x*TS,Y=y*TS;
    if(s.districtStage===1){
      px(c,X+5,Y+9,22,3,p.wood,.62);for(const dx of[8,15,22]){px(c,X+dx,Y+12,3,7,'#465338',.58);px(c,X+dx-1,Y+18,5,3,'#66744e',.38)}
    }else if(s.districtStage===2){
      px(c,X+6,Y+7,20,5,'#5e4637',.56);px(c,X+8,Y+12,16,8,'#514437',.34);px(c,X+15,Y+20,2,4,'#3c332b',.58);
    }else if(s.districtStage===3){
      px(c,X+7,Y+6,4,17,'#7a4b31',.58);px(c,X+8,Y+6,1,17,'#b26a3f',.48);px(c,X+7,Y+9,12,3,'#6d5439',.56);px(c,X+18,Y+8,3,6,'#7b6844',.48);
    }else if(s.districtStage===4){
      px(c,X+7,Y+7,18,12,'#17201f',.74);px(c,X+9,Y+9,14,8,'#41615f',.34);px(c,X+15,Y+9,2,8,'#a8d1ca',.22);
    }else{
      px(c,X+6,Y+8,20,4,'#735039',.58);px(c,X+15,Y+8,3,14,'#6f4a32',.62);c.strokeStyle='#9b7a50';c.lineWidth=2;c.beginPath();c.arc(X+16,Y+16,5,0,Math.PI*2);c.stroke();
    }
  }
}
...
function doorsAndStairs(c:CanvasRenderingContext2D,s:GameState){
  ...
  for(let y=0;y<s.height;y++)for(let x=0;x<s.width;x++){
    const t=s.tiles[y*s.width+x];if(!t?.visible)continue;const X=x*TS,Y=y*TS;
    if(t.kind==='door'){
      px(c,X+3,Y+27,26,4,'#050705',.70);px(c,X+4,Y+4,24,26,'#17140f',.95);px(c,X+7,Y+6,18,23,p.wood,.98);px(c,X+9,Y+8,14,3,'#9b7047',.48);px(c,X+9,Y+13,14,12,'#5a3f2c',.95);px(c,X+21,Y+18,3,3,'#d0aa61',.90);px(c,X+5,Y+3,22,2,p.edge,.40);
...
function playerPool(c:CanvasRenderingContext2D,s:GameState){
  // Soft local floor lift gives the hero a physical pool of light without revealing any hidden tile.
  for(let y=Math.max(0,s.player.y-3);y<=Math.min(s.height-1,s.player.y+3);y++)for(let x=Math.max(0,s.player.x-3);x<=Math.min(s.width-1,s.player.x+3);x++){
    const t=s.tiles[y*s.width+x];if(!t?.visible||t.kind==='wall')continue;
    const d=Math.abs(x-s.player.x)+Math.abs(y-s.player.y);if(d>3)continue;
    px(c,x*TS+2,y*TS+2,28,28,'#c9b57c',(4-d)*.012);
  }
}
