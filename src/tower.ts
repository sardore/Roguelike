import { contextSystem, hasIntrinsic, populateNethackFloor, preMoveSystem, searchSystem, syncNethack, systemMessage } from './nethack';
...
export function waitTurn(s:TowerState){if(s.dead||s.won)return;const act=contextSystem(s);if(act==='There is nothing here that needs your hands.'){s.hero.guard=Math.min(s.hero.cls==='vanguard'?6:4,s.hero.guard+1);systemMessage(s,searchSystem(s))}else systemMessage(s,act);endTurn(s)}
