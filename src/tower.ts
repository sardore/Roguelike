...
function log(s:TowerState,m:string){s.messages.push(m);if(s.messages.length>6)s.messages.shift()}
...
function attack(s:TowerState,e:Unit,bonus=0){let dmg=s.hero.power+bonus-(e.armor??0);if(e.elite)dmg=Math.max(1,dmg-1);dmg=Math.max(1,dmg);e.hp-=dmg;log(s,`You hit ${labelEnemy(e.kind)} for ${dmg}.`);if(e.hp<=0)kill(s,e)}
function canStand(s:TowerState,x:number,y:number){const t=get(s,x,y);return !!t&&isWalk(t.kind)&&!enemyAt(s,x,y)&&!(s.hero.x===x&&s.hero.y===y)}
...
function enemyTurn(s:TowerState){for(const e of [...s.enemies]){if(s.dead)return;if(resolveIntent(s,e))continue;if(e.cooldown>0){e.cooldown--;continue}const d=dist(e,s.hero),visible=lineClear(s,e,s.hero);
    ...
    if(e.kind==='slime'){if(d===1){rangedHit(s,e,3,'Caustic slime burns for {n}.');const t=get(s,e.x,e.y);if(t?.kind==='floor')t.kind='water'}else if(d<=6)stepToward(s,e);continue}
    ...
    if(e.kind==='wisp'){if(d===1)rangedHit(s,e,3,'Cold light sears for {n}.');else if(d<=6&&visible&&Math.random()<.35){for(let k=0;k<12;k++){const nx=s.hero.x+(Math.floor(Math.random()*7)-3),ny=s.hero.y+(Math.floor(Math.random()*7)-3);if(canStand(s,nx,ny)){e.x=nx;e.y=ny;break}}}else if(d<=8)stepToward(s,e);continue}
...
function stepHazard(s:TowerState){const t=get(s,s.hero.x,s.hero.y);if(t?.kind==='lava'){damageHero(s,5);log(s,'Crown-fire burns for 5.')}else if(t?.kind==='trap'){damageHero(s,4);t.kind='floor';log(s,'A hidden spike snaps upward for 4.')}}
function floorBossAlive(s:TowerState){return s.floor%4===0&&s.enemies.some(e=>e.kind==='boss')}
function endTurn(s:TowerState){s.turn++;if(s.hero.skillCooldown>0)s.hero.skillCooldown--;enemyTurn(s);updateVisibility(s)}
...
export function waitTurn(s:TowerState){if(s.dead||s.won)return;s.hero.guard=Math.min(s.hero.cls==='vanguard'?6:4,s.hero.guard+1);log(s,'You hold position.');endTurn(s)}
...
function opaque(t?:Tile){return !t||t.kind==='wall'||t.kind==='books'||t.kind==='pillar'}
function canSee(s:TowerState,x0:number,y0:number,x1:number,y1:number){let x=x0,y=y0,dx=Math.abs(x1-x0),sx=x0<x1?1:-1,dy=-Math.abs(y1-y0),sy=y0<y1?1:-1,err=dx+dy;while(!(x===x1&&y===y1)){const e2=2*err;if(e2>=dy){err+=dy;x+=sx}if(e2<=dx){err+=dx;y+=sy}if(x===x1&&y===y1)return true;if(opaque(get(s,x,y)))return false}return true}
