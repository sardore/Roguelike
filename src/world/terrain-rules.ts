...
function terrainPool(theme:ThemeDefinition):Array<{value:TileKind;weight:number}>{
  ...
  if(tags.has('aquatic')){add('water',5);add('ice',1);}
  if(tags.has('fire')){add('lava',4);add('oil',3);add('rubble',1);}
  if(tags.has('ice')){add('ice',6);add('water',2);}
  if(tags.has('venom')){add('miasma',5);add('bramble',2);}
  if(tags.has('fungal')){add('miasma',3);add('bramble',4);}
  if(tags.has('plant'))add('bramble',5);
  ...
  if(tags.has('royal')||tags.has('humanoid'))add('holy',1);
  if(tags.has('construct')){add('oil',2);add('rubble',2);}
  if(!out.length){add('rubble',2);add('water',1);}
  return out;
}
...
export function applyThemedTerrain(floor:FloorMap,theme:ThemeDefinition,coord:WorldCoord,rng:DeterministicRng,extraReserved:Point[]=[]):void{
  if(theme.id==='abyss'&&rng.chance(.7)){
    theme={...theme,monsterTags:[...theme.monsterTags,'void','aberrant']};
  }
  const reserved=new Set<string>([key(floor.spawn),...floor.exits.map(key),...extraReserved.map(key)]);
  const candidates:Point[]=[];
  for(let i=0;i<floor.tiles.length;i+=1){const tile=floor.tiles[i]!;if(tile.kind!=='floor'&&tile.kind!=='rubble')continue;const point={x:i%floor.width,y:Math.floor(i/floor.width)};if(reserved.has(key(point))||manhattan(point,floor.spawn)<5||floor.exits.some((exit)=>manhattan(point,exit)<3))continue;candidates.push(point);}
  if(!candidates.length)return;
  const pool=terrainPool(theme),patches=Math.min(6,2+Math.floor(coord.depth/35)+rng.int(0,2));
  for(let patch=0;patch<patches;patch+=1){
    ...
    for(let step=0;step<steps;step+=1){
      for(let y=Math.max(1,cursor.y-radius);y<Math.min(floor.height-1,cursor.y+radius+1);y+=1)for(let x=Math.max(1,cursor.x-radius);x<Math.min(floor.width-1,cursor.x+radius+1);x+=1){
        const point={x,y};if(manhattan(point,cursor)>radius||reserved.has(key(point)))continue;const index=y*floor.width+x,tile=floor.tiles[index]!;if(tile.kind!=='floor'&&tile.kind!=='rubble')continue;if(rng.chance(.52))floor.tiles[index]=terrainTile(kind);
      }
